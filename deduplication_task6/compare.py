...
class DateProximityComparison(BaseCompareFeature):
    """Compare date attributes of record pairs based on proximity in years.
    ...
    right_on : str or int
        The name or position of the column in the right DataFrame.
    agree_value : int or float, optional
        The value for an agreement. Default 1.
    disagree_value : int or float, optional
        The value for a disagreement. Default 0.
    missing_value : int or float, optional
        The value for a missing comparison. Default 0.
    label : str, optional
        Optional label for the feature.
    """

    name = "date_proximity_comparison"
    description = "Compare date attributes of record pairs based on proximity in years."
    ...
    def _compute_vectorized(self, s_left, s_right):
        ...
        # Calculate the absolute difference in days and convert to years
        difference_in_years = abs((s_left - s_right).dt.days / 365.25)

        # Apply the condition for the year difference threshold
        c = (difference_in_years <= 10).astype(numpy.int64)

        # Convert the boolean result to agree or disagree values
        c = c * self.agree_value + (~c.astype(bool)) * self.disagree_value
...
class DateSwap(BaseCompareFeature):
    """Compare date attributes of record pairs based on complex proximity rules.
    ...
    name = "complex_date_comparison"
    description = "Compare date attributes of record pairs based on complex proximity rules."
    ...
    def _compute_vectorized(self, s_left, s_right):
        ...
        df = pandas.DataFrame({'left': s_left, 'right': s_right})

        # Apply complex conditions
        condition1 = df['left'] == df['right']
        condition2 = ((df['right'].dt.year - 1900 == df['left'].dt.day) &
                      (df['right'].dt.day == (df['left'].dt.year - 1900)) &
                      (df['right'].dt.month == df['left'].dt.month))
        condition3 = ((df['right'].dt.month == (df['left'].dt.year - 1900)) &
                      ((df['right'].dt.year - 1900) == df['left'].dt.month) &
                      (df['right'].dt.day == df['left'].dt.day))
        condition4 = ((df['right'].dt.year == df['left'].dt.year) &
                      (df['right'].dt.day == df['left'].dt.month) &
                      (df['right'].dt.month == df['left'].dt.day))

        # Combine all conditions
        final_condition = condition1 | condition2 | condition3 | condition4

        # Convert the boolean result to agree or disagree values
        c = final_condition.astype(numpy.int64) * self.agree_value + (~final_condition.astype(bool)) * self.disagree_value
