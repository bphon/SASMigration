...
class Block(BaseIndexAlgorithm):
    ...
    not_equal_on : tuple of (left_label, right_label), optional
        A tuple specifying the columns that should not be equal in the resulting pairs.
    **kwargs :
        Additional keyword arguments to pass to
        :class:`recordlinkage.base.BaseIndexAlgorithm`.
    ...
    def __init__(self, left_on=None, right_on=None, not_equal_on=None, **kwargs):
        super().__init__(**kwargs)

        # variables to block on
        self.left_on = left_on
        self.right_on = right_on if right_on is not None else left_on
        self.not_equal_on = not_equal_on
    ...
    def _link_index(self, df_a, df_b):
        ...
        data_right["index_y"] = df_b.index

        # merge the dataframes
        pairs_df = data_left.merge(data_right, how="inner", on=blocking_keys)

        # apply not equal condition
        if self.not_equal_on:
            left_not_equal_on, right_not_equal_on = self.not_equal_on
            # Include the not_equal_on columns in the pairs_df for filtering
            pairs_df = pairs_df.merge(df_a[[left_not_equal_on]], left_on="index_x", right_index=True)
            pairs_df = pairs_df.merge(df_b[[right_not_equal_on]], left_on="index_y", right_index=True)
            pairs_df = pairs_df[pairs_df[left_not_equal_on] != pairs_df[right_not_equal_on]]
