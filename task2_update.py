...
date_columns = ['diag_dte', 'DoB', 'dth_dte']
for col in date_columns:
    d1[col] = pd.to_datetime(d1[col], errors='coerce').dt.strftime('%Y-%m-%d')

# Filter by diagnosis date
current_date = datetime.now().strftime('%Y-%m-%d')
d1 = d1[(d1['diag_dte'] >= '1980-01-01') & (d1['diag_dte'] <= current_date)]
...
Criteria
  ...
  Age at diagnosis >= 18
  Any Residence
  Cases diagnosed between 01JAN1980 and (approximately) {current_date}
"""
...
def send_email(subject, body, to_emails, attachments=[]):
    ...
    with smtplib.SMTP('mail.albertahealthservices.ca') as server:
...
if myerror == 0:
    ...
    # Send log file email
    send_email(
        subject=f"Cervix Log from {current_date}",
        body=f"This is the cervix run log file for {current_date}.",
    ...
    # Send results email
    send_email(
        subject=f"Cervix Results from {current_date} 2014-009",
        body=f"""
        Dear All,
    ...
    # Send error email
    send_email(
        subject=f"ERRORS in Cervix run {current_date} 2014-009",
        body=f"The cervix run log of {current_date} contained errors. No data was sent. The log file is attached.",
        to_emails=["angela.eckstrand@albertahealthservices.ca", "tuyet.thieu@albertahealthservices.ca"],
        attachments=[log_path]
    )
